...
int vm_fault(void *addr, bool write_flag)
{
    ...
    if(processtable[curprocess]->pageq[pageqindex]->residentbit!=0)// if it is not resident
    {
        page* respage = processtable[curprocess]->pageq[pageqindex];
        if(!write_flag)
        {
            if(respage->dirtybit==1)// resident page is modified
            {
                pte->write_enable =1;
            }
        }
...
int vm_syslog(void *message, unsigned int len)
{
    ...
    for(unsigned int i=0;i<len;i++)
    {
        ...
        {
            if(vm_fault((void*)(vaddrin+i), false )==-1) return -1;
        }
